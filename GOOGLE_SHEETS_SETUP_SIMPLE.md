...
## 🔧 Быстрое исправление текущей проблемы:

Если вы используете URL с `/dev`:

1. **Google Apps Script:**
   - Deploy → Manage deployments
   - Найдите последний deployment
   - Скопируйте URL, который заканчивается на `/exec`

2. **Railway:**
   - Variables → `GOOGLE_SHEETS_WEBHOOK_URL`
   - Замените URL на тот, что заканчивается на `/exec`
   - Сохраните

3. **Обновите код Google Apps Script:**
   - Добавьте функцию `doGet` (см. код выше) для тестирования
   - Save → Deploy → Update (или New deployment)

4. **Проверьте:**
   - Откройте URL с `/exec` в браузере → должно показать сообщение об успехе
   - Отправьте тестовый email → проверьте Railway логи
